function Select-EFMenuControlId {
    ...
    Write-EFMenuLine -Text 'Select automatic controls' -Color Cyan -NoColor:$NoColor -Width $Width
    for ($index = 0; $index -lt $candidates.Count; $index++) {
        $candidate = $candidates[$index]
        Write-EFMenuLine -Text ("{0}. {1} - {2}{3}" -f ($index + 1), $candidate.ControlId, $candidate.Title, `
            $(if ($candidate.RequiresReboot) { ' [restart may be required]' } else { '' })) `
            -NoColor:$NoColor -Width $Width -Indent 2
    }

    while ($true) {
        $selection = Read-EFMenuInput -Prompt 'Enter numbers separated by commas, A for all, or B to cancel'
        ...
        Write-EFMenuLine -Text '[INVALID] Choose listed numbers, A, or B.' -Color Yellow -NoColor:$NoColor -Width $Width
    }
}
